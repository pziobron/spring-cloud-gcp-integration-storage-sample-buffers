...
public class ImprovedGcsSession implements Session<BlobInfo> {
  ...
  private static final Log LOGGER = LogFactory.getLog(ImprovedGcsSession.class);

  private static final String SEPARATOR = "/";

  public ImprovedGcsSession(Storage gcs) {
  ...
  public void write(InputStream inputStream, String destination) throws IOException {
    ...
    int bufferSizePerFile = 15360; //15 MiB
    if (available > bufferSizePerFile) {
      try (InputStream is = inputStream; ReadableByteChannel inputChannel = Channels.newChannel(is)) {
        ByteBuffer buffer = ByteBuffer.allocateDirect(bufferSizePerFile);
